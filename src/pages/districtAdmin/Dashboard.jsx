...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (selectedLevel == null && classLevels?.length > 0) {
      const defaultLevel = classLevels?.[1].id;
  ...
  const options = classLevels?.map((level) => ({
    value: level?.id,
    label: level?.name,
    isDisabled: level?.id === 1, // This disables the Ruby Level (id: 1)
  }));
      ...
      <div className="top-head">
        <div className="top-head-in">
          ...
          <p className="top-head-p">
            {
              allSchoolList?.find((school) => school.id == selectedSchool)
                ?.school
            }{" "}
            Completion
          </p>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <Select
            ...
            styles={{
              ...
              option: (base, state) => ({
                ...base,
                // 2. Add visual feedback for the disabled state
                backgroundColor: state.isDisabled
                  ? "#f0f0f0" // Light grey background for disabled
                  : state.isFocused
                  ? "#4126A8"
                  : "white",
                color: state.isDisabled
                  ? "#999" // Light grey text for disabled
                  : state.isFocused
                  ? "white"
                  : "#333",
                cursor: state.isDisabled ? "not-allowed" : "pointer",
                "&:active": {
                  backgroundColor: state.isDisabled ? "#f0f0f0" : "#4126A8",
                },
              }),
            }}
            options={options}
            ...
            onChange={(selected) => {
              // 3. Optional: Safety check to ensure the function doesn't run if disabled
