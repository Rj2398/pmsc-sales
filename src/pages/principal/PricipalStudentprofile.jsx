	...
	return (
		<div>
			...
			<div className="student-short-info">
				<h3> {studentProfileInfo?.student_name} 
					<span style={{width:"52px"}}></span> 
				</h3>
				<p> {studentProfileInfo?.student_email} 
					...
					<div className="table-responsive">
						<table>
							...
							<tbody>
								...
								<td>
									<Link to={ principalComing ? "/principal/student/baseline/assesment"
										: teacherStudentComing ? "/principal/student-baseline/assesment"
										: principalstudentBaseline ? "/district-admin-student-baseline/assesment" : "/principal/student-baseline-assesment"
									}
									state={{ studentId: studentId, subjectId: selectedCourses?.length > 0 ? selectedCourses?.[0] : subjectId, }} >
								...
								<tr>
									...
									<td>&nbsp;</td>
									<td>
										<div className={`status ${["not_started", "locked", ""].includes(studentProfileInfo?.lesson_overall_status) && "inactive"} ${["not_completed", "in_progress","retake"].includes(studentProfileInfo?.lesson_overall_status) && "review"}`} >{studentProfileInfo?.lesson_overall_status?.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()) || "Not Started"}</div>
									</td>
									<td>
								...
								{studentProfileInfo?.lesson_wise?.map((lesson, index) => (
									<tr key={index} className="lessons-list" style={showAllLession ? {} : { display: "none" }} >
										...
										<td>
											<div className={`status ${["not_started", "locked"].includes(lesson?.status) && "inactive"} ${["not_started", "not_completed", "in_progress"].includes(lesson?.status) && "review"}`}>
												{lesson?.status.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())}	
											</div>
										</td>
					
								{showFullDetails && !["not_started", "not_completed", "in_progress", "locked"].includes(lesson?.status) && (
								<td>
									<Link to={
										principalComing ? `/principal/student/lesson/quiz/:studentId=${studentId}`
								...
								<tr>
									...
									<td>
										<div className={`status ${["not_started", "locked"].includes(studentProfileInfo?.summative_status) && "inactive"} ${["not_completed", "in_progress","retake"].includes(studentProfileInfo?.summative_status) && "review"}`} >
											{studentProfileInfo?.summative_status?.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()) || "Not Started"}
										</div>
									</td>
												
									{showFullDetails && !["not_started", "not_completed", "in_progress", "locked"].includes(studentProfileInfo?.summative_status) && (
										<td>
											<Link to={ principalComing ? "/principal/student/summative" : teacherStudentComing ? "/principal/students-summative" : principalstudentBaseline ? "/district-admin-student-summative" : "/principal/student-summative" }
											state={{ studentId: studentId, subjectId: selectedCourses?.length > 0 ? selectedCourses?.[0] : subjectId, }} >
												<i className="fa-light fa-eye"></i> View Full Details
											</Link>
										</td>
