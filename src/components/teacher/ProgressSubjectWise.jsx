				...
				<table>
					{(selectedStudents?.length > 0 && selectedCourses?.length > 0) ? 
						...
						<tr>
							...
							<td>
								<div className={`status ${(subjectWizeScoreData?.[0]?.baseline_status == "not_started" || !subjectWizeScoreData?.[0]?.baseline_status) && "inactive"} ${["not_completed", "in_progress"].includes(subjectWizeScoreData?.[0]?.baseline_status) && "review"}`}>{subjectWizeScoreData?.[0]?.baseline_status ? subjectWizeScoreData?.[0]?.baseline_status.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()) : "Not Started"}</div>
							</td>
							{!subjectWizeScoreData?.[0]?.baseline_status || !["not_started", "not_completed", "in_progress", "locked"].includes(subjectWizeScoreData?.[0]?.baseline_status)  && <td>
						...
						<tr>
							...
							<td> &nbsp; </td>
							<td>
								<div className={`status ${(subjectWizeScoreData?.[0]?.lesson_overall_status == "not_started" || subjectWizeScoreData?.[0]?.lesson_overall_status == "locked" || !subjectWizeScoreData?.[0]?.baseline_status) && "inactive"} ${["not_started", "not_completed", "in_progress"].includes(subjectWizeScoreData?.[0]?.lesson_overall_status) && "review"}`}>{subjectWizeScoreData?.[0]?.lesson_overall_status ? subjectWizeScoreData?.[0]?.lesson_overall_status.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()) : "Not Started"}</div>
							</td>
						</tr>
						...
						{subjectWizeScoreData?.[0]?.lesson_wise?.map((item, index) => (
							<tr className="lessons-list" style={{ display: lessonToggle ? "" : "none" }} key={index}>
								...
								<td>
									<div className={`status ${(item?.status =="locked" || !item?.status) && "inactive"} ${["not_completed", "in_progress"].includes(item?.status) && "review"} ${item?.status == "not_started" && "inactive"}`}>{item?.status?.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()) || "Not Started"}</div>
								</td>
						...
						<tr>
							...
							<td>
								<div className={`status ${(subjectWizeScoreData?.[0]?.summative_status =="locked" || !subjectWizeScoreData?.[0]?.summative_status) && "inactive"}  ${["not_completed", "in_progress"].includes(subjectWizeScoreData?.[0]?.summative_status) && "review"}`}>{subjectWizeScoreData?.[0]?.summative_status ? subjectWizeScoreData?.[0]?.summative_status?.replace(/_/g, " ")?.replace(/\b\w/g, (char) => char?.toUpperCase()) : "Not Started"}</div>
							</td>
							{!subjectWizeScoreData?.[0]?.summative_status || !["not_started", "not_completed", "in_progress", "locked"].includes(subjectWizeScoreData?.[0]?.summative_status) && <td>
